...
def fc_op_1(input_op, input_center_op, imput_neighbor_op, name, n_out, layer_collector, act_func=tf.nn.leaky_relu):
    n_in = input_op.get_shape()[-1].value
    with tf.name_scope(name) as scope:
...
class SDNE(object):
    def __init__(self, graph, encoder_layer_list, alpha=1e-6, beta=2.0, nu1=1e-5, nu2=1e-4,
                 learning_rate=None):
        """
        ...
        if self.lr is None:
            self.lr = tf.train.inverse_time_decay(0.001, self.global_step, decay_steps=1, decay_rate=0.00005)
        ...
        self.center_AdjBatch = self.AdjBatch[:int(config.batch_size_dis//2), :]
        self.neighbor_AdjBatch = self.AdjBatch[int(config.batch_size_dis//2):, :]
...
#         self.neighbor_AdjBatch = tf.placeholder(tf.float32, [None, self.node_size], name='neighbor_adj_batch')
        ...
        with tf.name_scope(scope_name):
            for i in range(1, self.encoder_layer_num):
                fc, center_fc, neighbor_fc = fc_op_1(fc, center_fc, neighbor_fc,
                           name=scope_name + str(i),
                           n_out=self.encoder_layer_list[i],
                           layer_collector=layer_collector)

        self._embeddings = fc
        # self.center_embedding = fc[ :int(config.batch_size_dis//2), :]
        # self.neighbor_embedding = fc[int(config.batch_size_dis//2): , :]
        self.center_embedding = center_fc
        self.neighbor_embedding = neighbor_fc
        score = tf.reduce_sum(tf.multiply(self.center_embedding, self.neighbor_embedding), axis=1)
        self.score_gen = np.multiply(score,self.T)
        self.score_dis = np.divide(score,self.T)
        ...
        self.L_2nd = L_2nd
        L_dis = tf.reduce_sum(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=self.label, logits=self.score_dis)) #+ config.lambda_dis * (tf.nn.l2_loss(self.neighbor_embedding) + tf.nn.l2_loss(self.center_embedding))
        self.L_dis = L_dis
#         self.L = L_2nd + 0.0001*(L_1st) + 0.001*L_dis
        self.L =L_2nd + L_dis
#     +config.lambda_dis * (
#                 tf.nn.l2_loss(self.neighbor_embedding) +
#                 tf.nn.l2_loss(self.center_embedding))
        ...
        self.score_gen_c = tf.clip_by_value(self.score_gen, clip_value_min=-10, clip_value_max=10)
        self.reward = tf.math.log(1 + tf.math.exp(self.score_gen_c))
...
#             reward = self.sess.run(self.reward, feed_dict=feed_dict)
            cpt_size = len(node_1)
            cpt_over = cpt_size%int(config.batch_size_dis)
            cpt_size = cpt_size-cpt_over
            cpt_list = list(range(0,cpt_size,int(config.batch_size_dis//2)))   #计算reward分批
#             # cpt_max_iter = cpt_size // config.batch_size_dis  # batch_size=128
#             # print("total iter: %i" % cpt_max_iter)
            reward = []
            for cpt in cpt_list:
                end = cpt+int(config.batch_size_dis//2)

                node_1_t = np.asarray(node_1[cpt:end], dtype=np.int)
                node_2_t = np.asarray(node_2[cpt:end], dtype=np.int)
                node_mix = np.append(node_1_t, node_2_t)
                adj_batch_train = self.adj_mat[node_mix, :]
                
                adj_center = self.adj_mat[node_1_t, :]
                adj_cen_neighbir = adj_center[:, node_2_t]
                adj_t = np.ones_like(adj_cen_neighbir)
                adj_t[adj_cen_neighbir != 0] = self.beta
                score_t = np.diag(adj_t)
                            
                feed_dict = {self.AdjBatch: adj_batch_train,
                            self.T: score_t}
                reward_bch = self.sess.run(self.reward, feed_dict=feed_dict)
...
#                                                         })
            ...
            max_iter = len(center_index) // config.batch_size_dis
            print("total iter: %i" % max_iter)
            for step in range(max_iter):

#                 index_1 = np.random.randint(self.node_size, size=int(config.batch_size_dis//2)) #随机采样
                index_2 = np.random.randint(len(center_index), size=int(config.batch_size_dis//2)) #popred data for d 随机采样的索引
                center_index_t = center_index[index_2]
                neighbor_index_t = neighbor_index[index_2]
                index_mix = np.append(center_index_t , neighbor_index_t)
                ...
                adj_center = self.adj_mat[center_index_t , :]
                adj_cen_neighbir = adj_center[:, neighbor_index_t]
                adj_t = np.ones_like(adj_cen_neighbir)
                adj_t[adj_cen_neighbir != 0] = self.beta
                score_t = np.diag(adj_t)
