...
class GraphGAN(object):
    ...
    def prepare_data_for_g(self):
        ...
        for i in self.root_nodes:
            if np.random.rand() < config.update_ratio:
                sample, paths_from_i = self.sample(i, self.trees[int(i)], config.n_sample_gen, for_d=False)
    ...
    def sample(self, root, tree, sample_num, for_d):
        ...
        while len(samples) < sample_num:
            ...
            while True:
                ...
                relevance_probability = all_score[np.asarray(current_node,dtype=int), np.asarray(node_neighbor,dtype=int)]
