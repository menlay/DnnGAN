...
class SDNE(object):
    def __init__(self, graph, encoder_layer_list, alpha=1e-6, beta=5.0, nu1=1e-5, nu2=1e-4,
        ...
        if self.lr is None:
            self.lr = tf.train.inverse_time_decay(0.0001, self.global_step, decay_steps=1, decay_rate=0.5)
    ...
    def train(self, train_tuple,for_g=False):
        if for_g:
            ...
            for step in range(max_iter):
                ...
                index = np.random.randint(self.node_size, size=config.batch_size_dis)
                ...
                intersec = np.intersect1d(center_index,index)  #两数组交集
                select_index = []
                if len(intersec)>0:
                    for i in intersec:
                ...
                label_t = np.asarray(labels[select_index], dtype=np.float)
