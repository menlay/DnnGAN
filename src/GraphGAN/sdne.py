...
class SDNE(object):
    def __init__(self, graph, encoder_layer_list, alpha=1e-6, beta=5., nu1=1e-5, nu2=1e-4,
                 batch_size=20, epoch=20000, learning_rate=None):
        """
        ...
        if self.lr is None:
            self.lr = tf.train.inverse_time_decay(0.03, self.max_iter, decay_steps=1, decay_rate=0.9)
            ...
            if step % 50 == 0:
                l, l1, l2 = self.sess.run((L, L_1st, L_2nd),
                                          feed_dict={AdjBatch: adj_batch_train,
                                                     Adj: adj_mat_train,
                                                     B: b_mat_train})
                embedding = self.sess.run(_embeddings, feed_dict={AdjBatch: adj_mat})
                self.vectors = {}
                look_back = self.g.look_back_list
                for i, emb in enumerate(embedding):
                    self.vectors[look_back[i]] = emb
                self.evaluation_new(config.train_filename, self.vectors, config.n_emb)
    ...
    def evaluation_new(self, train_file, data, dim):
        panda_result = mt.eval_NE(train_file,data,dim,config.result_filename)
