...
class LPEvaluator(object):
    ...
    def _evaluate_ne_cmd(self, data_split, method_name, command, edge_embedding_methods, input_delim, output_delim,
        ...
        # tmpedg = './edgelist.tmp'
        # tmpemb = './emb.tmp'
        tmpedg = '/home/ming/gittest/EvalNE/examples/edgelist.tmp'
        tmpemb = '/home/ming/gittest/EvalNE/examples/emb.tmp'
        ...
        try:
            ...
            if os.path.isfile(tmpedg):
                os.remove(tmpedg)
            if 'grarep' in command:
                if os.path.isfile('/home/ming/gittest/EvalNE/examples/pretrain.emb'):
                    os.remove('/home/ming/gittest/EvalNE/examples/pretrain.emb')
                shutil.copy(tmpemb, '/home/ming/gittest/EvalNE/examples/pretrain.emb')
    ...
    def evaluate_cmd(self, method_name, method_type, command, edge_embedding_methods, input_delim, output_delim,
        ...
        if tune_params is not None:
            ...
            for i in range(len(edge_embedding_methods)):
                ...
                logging.info('Validation score for method `{}_{}` is: {}, corresponding best params were: `{}`'
                             .format(method_name, edge_embedding_methods[i], bestscore, best_params[i]))
                print('Validation score for method `{}_{}` is: {}, corresponding best params were: `{}`'
                             .format(method_name, edge_embedding_methods[i], bestscore, best_params[i]))
...
#         X = np.array(X.values())
#         X = X[np.argsort(keys), :]
        keys = list(map(int, X.keys()))
        X = np.vstack((X[str(i)] for i in sorted(keys)))
